...
if (steam_check($steam_id))
    $user_info = new User_Info($steam_id);
?>
<aside class="col-auto col-sm-4 col-md-3 col-xl-3 col-xxl-2 p-0 panel-sidebar" id="aside">
    <section class="user-panel">
        <img src="<?php if (steam_check($steam_id)) echo $user_info->steamprofile['avatarfull']; else echo assets("images/icons/user-1.png"); ?>" alt="user-image" class="user-img">
        <a href="<?php ?>" class="user-info"><?php if (isset($fname)) echo $fname; if (isset($lname)) echo " " . $lname; if (steam_check($steam_id)) echo ' ( ' . $user_info->steamprofile['personaname'] .' )'; ?></a>
        <p class="user-role">سطح کاربری : <?php echo $role_name; ?></p>
        <section class="user-steam-check"><p>اتصال به استیم :</p><p style="padding-right: .2rem;<?php if (steam_check($steam_id)) echo 'color:#2ECC71;'; else echo 'color:#E74C3C;'; ?>"><?php echo steam_status($steam_id); ?></p></section>
    </section>
    <nav>
        <ul>
            ...
            if ( Authorization::check( "UserViewOther" ) ) { ?>
                ...
                <?php } ?>
            <li class="items menu<?php if ($activePage == "item-list" || $activePage == "add-item" || $activePage == "all-item-list"|| $activePage == "show-inventory" || $activePage == "add-game" || $activePage == "add-gift-card" || $activePage == "product_liked" || $activePage == "product_bookmarked" ) echo " menu-active"; ?>"><a href="javascript:void(0)"><span class="fad fa-store"></span><p>محصولات</p><span class="fas fa-angle-right icon-more"></span></a>
                <ul class="more<?php if ($activePage == "item-list" || $activePage == "add-item" || $activePage == "show-inventory" || $activePage == "all-item-list" || $activePage == "add-game" || $activePage == "add-gift-card" || $activePage == "product_liked" || $activePage == "product_bookmarked" ) echo " show"; ?>">
                    ...
                    <?php if ( Authorization::check( "ItemAdd" ) ) { ?><li class="items<?php if ($activePage == "show-inventory") echo " active"; ?>"><a href="<?php echo account("show-inventory.php"); ?>"><span class="fas fa-table"></span><p>آیتم های استیم شما</p></a></li><?php } ?>
